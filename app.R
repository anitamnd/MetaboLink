      ...
      fluidRow(
        ...
        bsCollapse(id = "menu", multiple = FALSE, open = "Data input",
          ...
          bsCollapsePanel("Blank filtration", style = "primary",
            fluidRow(style = "padding: 0px;",
              column(12,
                sliderInput("xbf", "Signal strength above blank", 0, 10, 5, step = 0.1, width = "100%"), style = "padding: 0px")
            ),
            fluidRow(style = "margin-right: 0px;",
          ...
          bsCollapsePanel("Missing value filtration", style = "primary",
                          fluidRow(style = "padding: 0px;",
                                   column(12,
                                          sliderInput("mvf_cutoff", "Minimum percentage of messured values", 0, 100, 80, step = 5, width = "100%"), style = "padding: 0px")
                          ),
                          fluidRow(column(6,
  ...
  dashboardBody(
    ...
    fluidRow(hidden(div(id = "buttons", style = "padding-bottom: 49px",
      ...
      column(3, bsButton("datatable", 
                         ...
                         style = "default",
                         block = T)),
      column(3, dropdownButton(inputId = "plot2", circle = F, width = "100%",
                               label = "Explore data",
    ...
    fluidRow(
      hidden(
        div(id = "sequence_panel",
          column(width = 8,
             box(title = textOutput("diboxtitle"), width = NULL,
                    fluidRow(
                               column(width = 4,
                                      h4("Sample")),
                               column(width = 2,
          ...
          column(width = 4,
             box(width = NULL, title = "Upload sequence file", status = "danger",
             ...
             box(width = NULL, title = "Extract adducts from name",
                 radioGroupButtons("add_mode","Select ionmode", c("Positive","Negative"), justified = T),
                 actionButton("add_run","Run", width = 100)
             ),
             box(width = NULL, title = "Edit data columns",
                 actionButton("seq_edit","Edit", width = 100)
             ),
             box(width = NULL, title = "Class nicknames",
                 actionButton("class_edit","Edit", width = 100)
             ),
             box(width = NULL, title = "Download sequence file",
    ...
    fluidRow(
      hidden(
        div(id = "pca_panel",
          fluidPage(
            fluidRow(column(6,selectInput("selectpca1","", choices = NULL, width = "100%")),
                     column(6,selectInput("selectpca2","", choices = NULL, width = "100%"))
            ),
            fluidRow(tabBox(tabPanel(title = "PCA",plotlyOutput("plotpca1")),
                            tabPanel(title = "QC-drift")),
                     tabBox(tabPanel(title = "PCA",plotlyOutput("plotpca2")))
            ),
            fluidRow(tabBox(tabPanel(title = "Første panel", htmlOutput("info1"))),
    ...
    fluidRow(
      hidden(
        div(id = "export_panel",
          ...
          box(title = ".xlsx", width = 4,column(12,
    ...
    fluidRow(
      hidden(
        div(id = "boxplot_panel",
          ...
          column(9, box(width = NULL, fluidRow(column(6, 
                        radioButtons(
...
server <- function(session, input, output) {
  ...
  observeEvent(input$in_file1, {
    try(dat <- read.csv(input$in_file1$datapath, header = 1, stringsAsFactors = F, check.names = F))
  ...
  observeEvent(input$class_edit, {
    showModal(
      modalDialog(title = "Edit class Nicknames", size = "s",easyClose = T,
  ...
  observeEvent(input$selectdata1, ignoreInit = T, {
    ...
    output$dt_drift_panel <- renderDT(rv$data[[rv$si]][rv$seq[[rv$si]][,1] %in% "Name"], rownames = F, options = list(
      autoWidth = T, scrollY = "700px", pageLength = 20))
    output$dt_boxplot_panel <- renderDT(rv$data[[rv$si]][rv$seq[[rv$si]][,1] %in% "Name"], rownames = F, options = list(
      autoWidth = T, scrollY = "700px", pageLength = 20))
  ...
  observeEvent(rv$choices, {
    ...
    output$seq_download <- downloadHandler(
      ...
      content = function(file) {
        write.csv(cbind("sample" = rownames(rv$seq[[rv$si]]),rv$seq[[rv$si]][,-1]), file, row.names = F)
      }
    )
  ...
  observeEvent(input$export_edit, {
    showModal(
      modalDialog(title = "Edit datasets", size = "m",easyClose = T,
                  ...
                  lapply(1:length(rv$choices), function(x){
                    fluidRow(
                      ...
                      column(width = 2, style = "text-align: left;",
                             prettyCheckbox(paste0("export_edit_keep",x),NULL, status = "info", value = T)),
                    )
                  }),
      )
    )
  ...
  observeEvent(input$add_run, {
    ...
    ionmode <- switch(input$add_mode, "Positive" = "Adduct_pos", "Negative" = "Adduct_neg")
  ...
  observeEvent(input$bf, {
    if(is.null(rv$si)) {
      ...
      bfdat <- blankfiltration(dat = rv$data[[rv$si]],
                               seq = bfseq,
                               xbf = input$xbf,
  ...
  observeEvent(input$is, {
    if(is.null(rv$si)) {
      ...
      isseq <- rv$seq[[rv$si]]
      isdat <- isfunc(dat = rv$data[[rv$si]],
                      seq = isseq,
                      is = input$isChoose,
  ...
  observeEvent(input$mvf_run, {
    if(is.null(rv$si)) {
      ...
      mvf_dat <- cutoffrm(dat = rv$data[[rv$si]],
                          seq = mvf_seq,
  ...
  observeEvent(input$imp_run, {
    if(is.null(rv$si)) {
      ...
      imp_dat <- imputation(dat = rv$data[[rv$si]],
                            seq = imp_seq,
  ...
  observeEvent(input$dc_run, {
    dc_seq <- rv$seq[[rv$si]]
    dc_dat <- driftcorrection(dat = rv$data[[rv$si]],
                              seq = dc_seq,
  ...
  observeEvent(input$md_rankings, {
    showModal(
      modalDialog(title = "Change the priority of annotations",size = "s",easyClose = T,
  ...
  observeEvent(input$md_run, {
    ...
    merged_dat <<- merge.func(dat1 = dat1,
                             seq1 = seq1,
                             dat2 = dat2,
                             seq2 = seq2,
                             ppmtol = input$md_ppm,
    ...
    cov <- cv(dub_qc)
    nclust <- sapply(dub_dat$mergeID, function(x){table(dub_dat$mergeID)[names(table(dub_dat$mergeID)) == x]})
    
    out_dub <- data.frame("nClust" = nclust,
    ...
    output$md_modal_dt <- renderDataTable({datatable(out_dub, rownames = F,
                                          options = list(dom = "t", autowidth = T, paging = F),
                                          selection = list(selected = finddup(out_dub,rankings))) %>% formatStyle(1:8, `border-top` = styleRow(cluster_ends, "solid 2px"))
                                          }, server = T)
    
    showModal(
      modalDialog(title = "Select features to keep",size = "l",
    ...
    if(!is.null(rv$si)) {
      ...
      if("Sample" %in% seq[,1]){
        ...
        if (sum(!is.na(sclass)) > 0) {
          classcv <- sapply(sort(unique(sclass)), function(x) {round(cvmean(sdata[,sclass %in% x]),2)})
  ...
  output$drift_ui <- renderUI({
    ...
    if(is.null(rv$si)) {
      ...
      lapply(1:length(input$dt_drift_panel_rows_selected), function(i){
        fluidRow(column(6,plotOutput(paste0("driftplotoutput",i),height = 280, width = "100%")),
  ...
  observe({
    if(length(input$dt_drift_panel_rows_selected)==0 && rv$drift_plot_select == 1) {
      output$driftplotoutput1 <- renderPlot({NULL})
      output$driftplotoutput21 <- renderPlot({NULL})
    } else if (rv$drift_plot_select == 1){
      for (i in 1:(length(input$dt_drift_panel_rows_selected)+1)) {
        output[[paste0("driftplotoutput",i)]] <- renderPlot({NULL})
        output[[paste0("driftplotoutput2",i)]] <- renderPlot({NULL})
      }
      for (i in 1:length(input$dt_drift_panel_rows_selected)) {
        local({
          my_i <- i
          output[[paste0("driftplotoutput",my_i)]] <- renderPlot({
            driftplot(data = rv$data[[rv$si]][input$dt_drift_panel_rows_selected[my_i],],
                      seq = rv$seq[[rv$si]])
          })
        })
        if(input$drift_select != "None"){
          local({
            ...
            output[[paste0("driftplotoutput2",my_i)]] <- renderPlot({
              driftplot(data = rv$data[[which(rv$choices %in% input$drift_select)]][input$dt_drift_panel_rows_selected[my_i],],
                              seq = rv$seq[[rv$si]])
            })
          })
        }
      ...
      output$cvscatterplot <- renderPlot({
        cvscatterplot(data = rv$data[[rv$si]],
      ...
      for (i in 1:length(input$dt_boxplot_panel_rows_selected)) {
        local({
          ...
          output[[paste0("boxplotoutput",my_i)]] <- renderPlot({
            myboxplot(data = rv$data[[rv$si]][input$dt_boxplot_panel_rows_selected[my_i],],
                      seq = rv$seq[[rv$si]],
  ...
  observe({
    if(!is.null(rv$si)) {
      seq <- rv$seq[[rv$si]]
      dat <- rv$data[[rv$si]]
      blank_mv <- sum(is.na(dat[seq[,1] %in% "Blank"])) + sum(dat[seq[,1] %in% "Blank"] == 0, na.rm = T)
      qc_mv <- sum(is.na(dat[seq[,1] %in% "QC"])) + sum(dat[seq[,1] %in% "QC"] == 0, na.rm = T)
      sample_mv <- sum(is.na(dat[seq[,1] %in% "Sample"])) + sum(dat[seq[,1] %in% "Sample"] == 0, na.rm = T)
      
      sdata <- dat[seq[,1] %in% "Sample"]
      sclass <- seq[seq[,1] %in% "Sample",][,4]
      if (sum(seq$lab %in% "QC") > 0) {
        qccv <- paste0("CV in QC samples: ", round(cvmean(dat[seq[,1] %in% "QC"]),2),"</br>")
      } else {
        qccv <- "No QC in dataset </br>"
      }
